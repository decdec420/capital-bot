...
async function checkRiskExits(
  ...
  if (decision.state !== "risk_exit" || !riskExit) return false;
...
async function checkSignals(
  ...
  if (decision.action === "hold") {
    const reason =
      decision.blockers[0] ?? decision.reasons[0] ?? decision.nextTrigger;
    await logTick(
      userId,
      settings.symbol,
      lastRsi,
      currentPrice,
      "hold",
      `${reason} — next: ${decision.nextTrigger}`,
    );
    return;
  }
  ...
  if (decision.state === "buy_ready" && !openTrade) {
    console.log(
      `[signal] ${userId} BUY — RSI ${lastRsi.toFixed(2)} < ${settings.rsi_buy_threshold} @ $${currentPrice.toFixed(2)}`,
    );
    try {
      if (!settings.live_trading) {
        ...
        await logTick(
          ...
          currentPrice,
          "buy",
          `PAPER BUY — RSI ${lastRsi.toFixed(1)}`,
        );
      } else {
        if (!state.credentials) throw new Error("live mode but no credentials");
      ...
      if (!settings.live_trading) {
        ...
        await sendTelegram(
          fmtSell(
            ...
            currentPrice,
            entry,
            (currentPrice - entry) * openTrade.size,
            pnlPct,
            false,
          ),
        );
        await logTick(
          ...
          currentPrice,
          "sell",
          `PAPER SELL — P&L $${((currentPrice - entry) * openTrade.size).toFixed(2)}`,
        );
      } else {
        if (!state.credentials) throw new Error("live mode but no credentials");
        const fill = await placeMarketSell(
